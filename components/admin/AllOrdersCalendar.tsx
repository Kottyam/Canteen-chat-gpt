...
const AllOrdersCalendar: React.FC = () => {
  const { users, orders, prices, holidays, addHoliday, deleteHoliday } = useData();
  const [currentDate, setCurrentDate] = useState(new Date());
  const [selectedDate, setSelectedDate] = useState<string | null>(null);
  const [message, setMessage] = useState('');
  const [saving, setSaving] = useState(false);

  const ordersByDate = useMemo(() => orders.reduce((acc, order) => {
  ...
  const isHoliday = (date: Date) => WEEKEND.includes(date.getDay()) || holidays.includes(formatDate(date));

  const toggleHoliday = async (dateString: string) => {
    const date = new Date(`${dateString}T00:00:00`);
    if (WEEKEND.includes(date.getDay())) { setMessage('Saturday and Sunday are automatically holidays.'); return; }
    setSaving(true); setMessage('');
    try {
      if (holidays.includes(dateString)) { await deleteHoliday(dateString); setMessage(`${dateString} holiday removed.`); }
      else { await addHoliday(dateString); setMessage(`${dateString} declared as holiday.`); }
    } catch (error:any) { console.error(error); setMessage(`Holiday update failed: ${error?.message || 'Unknown error'}`); }
  ...
  for(let day=1;day<=daysInMonth;day++){
    const date=new Date(currentDate.getFullYear(),currentDate.getMonth(),day); const dateString=formatDate(date); const holiday=isHoliday(date); const count=(ordersByDate[dateString]||[]).length; const today=formatDate(new Date())===dateString;
  ...
  return <div className="w-full min-w-0">
    ...
    <div className="mb-4 rounded-lg border bg-gray-50 p-3 text-sm text-gray-600">Saturday and Sunday are automatic holidays. Select a date to declare or remove another holiday.</div>
    ...
    <div className="grid grid-cols-1 gap-4 xl:grid-cols-3">
      <div className="min-w-0 rounded-lg bg-white p-3 shadow-sm sm:p-5 xl:col-span-2">
        ...
        {selectedDate&&<div className="mt-4 rounded-lg border bg-gray-50 p-3"><p className="font-semibold">Selected date: {selectedDate}</p><p className="mt-1 text-sm text-gray-600">{isHoliday(new Date(`${selectedDate}T00:00:00`))?'Holiday':'Working day'}</p>{!WEEKEND.includes(new Date(`${selectedDate}T00:00:00`).getDay())&&<button type="button" disabled={saving} onClick={()=>toggleHoliday(selectedDate)} className="mt-3 min-h-11 w-full rounded-lg bg-primary-600 px-4 py-2.5 text-sm font-semibold text-white disabled:opacity-50 sm:w-auto">{saving?'Saving…':holidays.includes(selectedDate)?'Remove Holiday':'Declare Holiday'}</button>}</div>}
      </div>
      <div className="min-w-0 rounded-lg bg-white p-3 shadow-sm sm:p-5">
        ...
        {selectedOrders.length?<div className="max-h-[32rem] space-y-3 overflow-y-auto">{selectedOrders.map(order=><div key={order.id} className="rounded-lg border p-3"><p className="font-bold text-gray-800">{employeeName(order.employeeId)}</p><p className="text-xs text-gray-500">SR: {order.employeeId}</p><ul className="mt-2 text-sm text-gray-600">{order.items.morningTea&&<li>Morning Tea</li>}{order.items.lunchMeals&&<li>Meals</li>}{order.items.lunchEgg&&<li>+ Egg</li>}{order.items.lunchFishMeat&&<li>+ Fish/Meat</li>}{order.items.eveningTea&&<li>Evening Tea</li>}</ul><p className="mt-2 text-right font-semibold">₹{calculateOrderTotal(order.items).toFixed(2)}</p></div>)}</div>:<p className="text-sm text-gray-500">{selectedDate?'No orders for this date.':'Select a date to see orders.'}</p>}
      </div>
    </div>
  </div>;
};
