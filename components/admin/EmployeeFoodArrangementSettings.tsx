...
const EmployeeFoodArrangementSettings:React.FC=()=>{
 ...
 const selectEmployee=(id:string)=>{if(saving)return;setSelectedId(id);applyDraft(id);setEditing(id!=='all');setMessage('','info')};
 ...
 const save=async()=>{
   ...
   try{
     await saveAdminEmployeeFoodContributionSetting(selectedId==='all'?null:selectedId,mode,numericPercentage,numericFixed);
     const rows=await loadAdminEmployeeFoodContributionSettings();setSettings(rows);applyDraft(selectedId,rows);setEditing(false);setMessage(selectedId==='all'?'All Employees food arrangement saved successfully.':'Individual employee override saved successfully.','success');
 ...
 const removeOverride=async(employeeId:string)=>{
   ...
   try{
     ...
     if(selectedId===employeeId){applyDraft(selectedId,rows);setEditing(false)}
 ...
 return <section className="rounded-xl border bg-white p-4"><div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between"><div><h4 className="font-bold">Employee Food Arrangement</h4><p className="mt-1 text-sm text-gray-500">Configure the All Employees default, then add independent employee overrides one by one. If no arrangement is configured, the member pays 100%. Fixed Amount is a monthly company-covered allowance, not an order limit.</p></div>{!editing&&<button type="button" disabled={loading||saving||locked||selectedId===''} onClick={startEdit} className="min-h-11 rounded-lg border px-5 font-semibold text-gray-700">Edit</button>}</div>
 <div className="mt-4 max-w-2xl space-y-4">
  <label className="block text-sm font-semibold text-gray-700">Employee / Add Individual Employee<select disabled={loading||saving} value={selectedId} onChange={e=>selectEmployee(e.target.value)} className="mt-1 block w-full rounded-lg border px-3 py-2.5 disabled:bg-gray-50"><option value="all">All Employees</option>{employees.map(e=><option key={e.identityId} value={e.identityId}>{e.name}</option>)}</select><span className="mt-1 block text-xs text-gray-500">Select an employee to add a new override or edit their existing saved override.</span></label>
