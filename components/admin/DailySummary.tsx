...
const DailySummary: React.FC = () => {
  ...
  const dynamicItems = useMemo(() => {
    ...
    day.forEach(o => Object.keys(o.items || {}).forEach(code => {
      if (!o.items[code]) return;
      const name = o.itemNames?.[code] || code;
      const item = result.get(code) || { name, count: 0 };
      item.count += Number(o.itemQuantities?.[code] || 1);
  ...
  return (
    <div className="w-full min-w-0">
      ...
      {!selectedDate ? (
        ...
        <>
          <div className="mb-5 grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-5">{dynamicItems.map(item => <div key={item.name} className="rounded-lg border bg-white p-3"><div className="text-xs text-gray-500">{item.name}</div><div className="mt-1 text-2xl font-bold text-gray-800">{item.count}</div></div>)}</div>
          <div className="mb-4 rounded-lg border bg-white p-4"><div className="flex flex-col gap-1 sm:flex-row sm:justify-between"><span className="font-semibold">Employees / Entries: {employeeCodes.length}</span><span className="font-bold text-primary-700">Total Collection: ₹{total.toFixed(2)}</span></div></div>
