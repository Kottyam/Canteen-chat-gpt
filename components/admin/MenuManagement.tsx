...
const MenuManagement: React.FC = () => {
  ...
  const [items, setItems] = useState<MenuItem[]>(menuItems.length ? menuItems.filter(i => !i.archived) : fallbackItems);
  ...
  const patch = (itemCode: string, next: Partial<MenuItem>) => setItems(prev => prev.map(item => item.itemCode === itemCode ? { ...item, ...next } : item));

  const save = async (item: MenuItem) => {
    const clean = { ...item, itemName: item.itemName.trim(), unitPrice: Number(item.unitPrice), archived: false };
    if (!clean.itemName || !Number.isFinite(clean.unitPrice) || clean.unitPrice < 0) { setMessage('Enter a valid menu name and price.'); return; }
    ...
    } catch (error: any) { console.error(error); setMessage(`Save failed: ${error?.message || 'Please check admin access.'}`); }
  ...
  const toggle = async (item: MenuItem) => {
    setBusy(item.itemCode);
    try {
      if (item.active) await deactivateMenuItem(item.itemCode); else await activateMenuItem(item.itemCode);
      const nextActive = !item.active;
      setItems(prev => prev.map(i => i.itemCode === item.itemCode ? { ...i, active: nextActive } : i));
      setMenuItems(prev => prev.map(i => i.itemCode === item.itemCode ? { ...i, active: nextActive, archived: false } : i));
      setMessage(item.active ? 'Menu item deactivated.' : 'Menu item activated.');
    } catch (error: any) { console.error(error); setMessage(`Status update failed: ${error?.message || 'Unknown error'}`); }
    finally { setBusy(null); }
  };

  const remove = async (item: MenuItem) => {
    if (!window.confirm(`Remove “${item.itemName}” from the menu? Existing order history will be preserved.`)) return;
    setBusy(item.itemCode);
    try {
      ...
      setMessage('Item archived. It will no longer appear in the menu, while old orders remain intact.');
    } catch (error: any) { console.error(error); setMessage(`Delete failed: ${error?.message || 'Unknown error'}`); }
    finally { setBusy(null); }
  };
  ...
  const add = async (event: React.FormEvent) => {
    ...
    if (!name || !Number.isFinite(price) || price < 0) { setMessage('Enter a valid menu name and price.'); return; }
    ...
    try {
      ...
      setItems(prev => [...prev, item]); setMenuItems(prev => [...prev, item]);
      setNewName(''); setNewPrice(''); setMessage('New menu item added permanently.');
    } catch (error: any) { console.error(error); setMessage(`Add failed: ${error?.message || 'Unknown error'}`); }
    finally { setBusy(null); }
  };
  ...
  return (
    <div className="w-full min-w-0">
      ...
      <p className="mb-5 mt-1 text-sm text-gray-500">Permanent menu templates. For date-wise menus, use the 7-Day Menu tab.</p>
      ...
      <div className="space-y-3">
        {items.map(item => (
          <div key={item.itemCode} className="rounded-xl border bg-white p-3 shadow-sm sm:p-4">
            <div className="space-y-3">
              <div><label className="mb-1 block text-xs font-semibold uppercase text-gray-500">Item name</label><input value={item.itemName} onChange={e => patch(item.itemCode, { itemName: e.target.value })} className="min-h-12 w-full rounded-lg border px-3 text-base outline-none focus:border-primary-500" /></div>
              <div><label className="mb-1 block text-xs font-semibold uppercase text-gray-500">Price</label><div className="flex min-w-0"><span className="flex min-h-12 shrink-0 items-center rounded-l-lg border border-r-0 bg-gray-50 px-4 text-gray-500">₹</span><input type="number" min="0" step="0.5" inputMode="decimal" value={item.unitPrice} onChange={e => patch(item.itemCode, { unitPrice: Number(e.target.value) })} className="min-h-12 w-full min-w-0 rounded-r-lg border px-3 text-lg outline-none focus:border-primary-500" /></div></div>
