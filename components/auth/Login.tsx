...
const Login: React.FC = () => {
    ...
    const looksLikeLegacyId = trimmedUserId === LEGACY_ADMIN_USER_ID
        || users.some(u => u.id === trimmedUserId)
        || (trimmedUserId.length === 5 && /^\d{5}$/.test(trimmedUserId));

    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        setError('');
        const normalized = normalizeMobileNumber(trimmedUserId);
        if (!looksLikeLegacyId && normalized && !/^[6-9][0-9]{9}$/.test(normalized)) {
            setError('Please enter a valid Mobile Number.');
            return;
        }
        const result = await loginWithCredentials(trimmedUserId, password, users);
        if (!result.ok) setError(result.error || (looksLikeLegacyId ? 'Admin ID or password is incorrect.' : 'Invalid Mobile Number or Password.'));
    };
      ...
      <div className="flex min-h-screen items-center justify-center bg-gray-50 px-4 py-8">
        <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-lg sm:p-8">
          ...
          <form className="mt-7 space-y-4" onSubmit={handleSubmit}>
            ...
            <input required inputMode="numeric" className="relative block min-h-12 w-full rounded-lg border border-gray-300 px-3 py-3 text-gray-900" placeholder={looksLikeLegacyId ? 'Admin User ID' : 'Mobile Number'} value={userId} onChange={e => setUserId(e.target.value)} />
