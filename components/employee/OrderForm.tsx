...
const EMPTY: OrderItems = { morningTea: false, lunchMeals: false, lunchEgg: false, lunchFishMeat: false, eveningTea: false };
...
const OrderForm: React.FC = () => {
  ...
  const todayMenu = useMemo<MenuItem[]>(() => {
    const scheduled = dailyMenus.filter(item => item.menuDate === today);
    if (scheduled.length) {
      return scheduled.filter(item => item.active).map(item => ({ itemCode: item.itemCode, itemName: item.itemName, unitPrice: item.unitPrice, active: true }));
    }
  ...
  const getScheduledItem = (key: keyof OrderItems) => todayMenu.find(i => i.itemCode === key);
  const getPrice = (order: Order | undefined, key: keyof OrderItems) => order?.itemPrices?.[key] ?? getScheduledItem(key)?.unitPrice ?? prices[key];
  const getName = (order: Order | undefined, key: keyof OrderItems) => {
    const fallback: Record<keyof OrderItems, string> = { morningTea: 'Morning Tea', lunchMeals: 'Meals', lunchEgg: 'Egg', lunchFishMeat: 'Fish/Meat', eveningTea: 'Evening Tea' };
    return order?.itemNames?.[key] ?? getScheduledItem(key)?.itemName ?? menuItems.find(i => i.itemCode === key)?.itemName ?? fallback[key];
  };

  const total = (order: Order | undefined) => order
    ? (Object.keys(order.items) as (keyof OrderItems)[]).reduce((sum, key) => sum + (order.items[key] ? getPrice(order, key) : 0), 0)
    : todayMenu.reduce((sum, item) => sum + (items[item.itemCode as keyof OrderItems] ? item.unitPrice : 0), 0);
  ...
  const save = async (event: React.FormEvent) => {
    ...
    try {
      const itemPrices = Object.fromEntries(
        (Object.keys(items) as (keyof OrderItems)[]).filter(k => items[k]).map(k => [k, getScheduledItem(k)?.unitPrice ?? prices[k]])
      );
      const itemNames = Object.fromEntries(
        (Object.keys(items) as (keyof OrderItems)[]).filter(k => items[k]).map(k => [k, getScheduledItem(k)?.itemName ?? menuItems.find(i => i.itemCode === k)?.itemName])
      );
      const order: Order = { id: crypto.randomUUID(), employeeId: user.id, date: today, items, itemPrices, itemNames };
      await upsertOrder(order, prices);
      setOrders(prev => [...prev, order]);
      setMsg('Today’s order saved successfully.');
    } catch (error: any) {
      console.error(error); setMsg(`Could not save order: ${error?.message || 'Please try again.'}`);
  ...
  const cancelToday = async () => {
    ...
    try {
      ...
      console.error(error); setMsg(`Could not cancel order: ${error?.message || 'Please try again.'}`);
    } finally { setSaving(false); }
  };
  ...
  if (existing) {
    return (
      <div className="w-full min-w-0 rounded-xl bg-white p-3 shadow-sm sm:p-6">
        ...
        <div className="space-y-2">{(Object.keys(existing.items) as (keyof OrderItems)[]).filter(k => existing.items[k]).map(k => <div key={k} className="flex justify-between rounded-lg border p-3"><span>{getName(existing, k)}</span><span>₹{getPrice(existing, k).toFixed(2)}</span></div>)}</div>
  ...
  return (
    <div className="w-full min-w-0 rounded-xl bg-white p-3 shadow-sm sm:p-6">
      ...
      {!holiday && <div className="mb-4 rounded-xl border border-green-100 bg-green-50 p-3 text-sm text-green-800">Today’s menu is set by the canteen admin. Prices shown here are the prices saved for today.</div>}
      <form onSubmit={save} className="space-y-3">
        {todayMenu.map(item => {
          const key = item.itemCode as keyof OrderItems;
          const inputId = `order-item-${item.itemCode}`;
          return <div key={item.itemCode} className={`flex min-h-14 items-center justify-between gap-3 rounded-xl border p-3 ${holiday ? 'opacity-60' : ''}`}>
            <div className="flex min-w-0 items-center gap-3"><input id={inputId} type="checkbox" disabled={holiday} checked={Boolean(items[key])} onChange={e => change(key, e.currentTarget.checked)} className="h-5 w-5 shrink-0 touch-manipulation" /><label htmlFor={inputId} className="min-w-0 cursor-pointer break-words text-sm font-medium text-gray-700 sm:text-base">{item.itemName}</label></div>
            <span className="shrink-0 text-sm font-semibold">₹{item.unitPrice}</span>
          </div>;
        })}
        {todayMenu.length === 0 && !holiday && <div className="rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-800">Today’s menu has not been published yet. Please check with the canteen admin.</div>}
        ...
        <button type="submit" disabled={saving || holiday || todayMenu.length === 0} className="min-h-12 w-full rounded-lg bg-primary-600 px-4 font-semibold text-white disabled:opacity-50">{holiday ? 'Ordering Closed' : saving ? 'Saving…' : 'Place Today’s Order'}</button>
