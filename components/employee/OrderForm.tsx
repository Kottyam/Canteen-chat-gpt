...
const OrderForm:React.FC=()=>{
 ...
 useEffect(()=>{
  let mounted=true;
  const check=async()=>{if(!mounted)return;try{const result=await fetchActiveEmployeeOrder();if(mounted){setServerEmployeeOrder(result);setCheckingEmployeeOrder(false)}}catch(error){console.error('Active employee order check failed',error);if(mounted){setServerEmployeeOrder(null);setCheckingEmployeeOrder(false);setMsg('Could not verify today’s employee order. Please try again.')}}};
 ...
 const save=async(e:React.FormEvent)=>{
  ...
  try{
   ...
   const order:Order={id:crypto.randomUUID(),employeeId:user.id,date:today,items:Object.fromEntries(chosen.map(i=>[i.itemCode,true])),itemQuantities,itemPrices,itemNames,orderSource:'employee',status:'active'};
