...
function tableHead(d:jsPDF,y:number){
  ...
  ['Date','Particulars','Quantity','Type','Total Amount'].forEach((h,i)=>{
    d.text(h,centerX(i),y+5.4,{align:'center'});
    if(i<4)d.line(COL_X[i]+COL_WIDTHS[i],y,COL_X[i]+COL_WIDTHS[i],y+HEADER_H);
  });
  return y+HEADER_H;
}

function rowHeight(d:jsPDF,t:RevenueTransaction){
  const particulars=split(d,t.particulars,COL_WIDTHS[1]);
  const type=split(d,t.type,COL_WIDTHS[3]);
  return Math.max(7.6,particulars.length*ROW_LINE+3,type.length*ROW_LINE+3,8.5);
}

function drawCenteredCell(d:jsPDF,text:string,column:number,y:number,h:number){
  const lines=split(d,text,COL_WIDTHS[column]);
  const blockH=lines.length*ROW_LINE;
  const startY=y+(h-blockH)/2+2.9;
...
function drawRow(d:jsPDF,t:RevenueTransaction,y:number){
  ...
  for(let c=1;c<5;c++)d.rect(COL_X[c],y,COL_WIDTHS[c],h);
  setBodyFont(d,false);
...
function drawDateCell(d:jsPDF,date:string,y:number,h:number){
  d.setDrawColor(...BORDER);d.setLineWidth(.2);d.rect(COL_X[0],y,COL_WIDTHS[0],h);
  setBodyFont(d,false);drawCenteredCell(d,formatReportDate(date),0,y,h);
}
...
function drawTransactions(d:jsPDF,r:MonthlyRevenueReport,y:number,m:number,yr:number,generatedAt:string){
  ...
  for(const[date,items]of groups){
    ...
    while(i<items.length){
      ...
      while(i<items.length){
        const h=rowHeight(d,items[i]);
        if(i>start&&y+used+h>BODY_BOTTOM)break;
        used+=h;i++;
      }
      const pageItems=items.slice(start,i);drawDateCell(d,date,y,used);let ry=y;
      pageItems.forEach(t=>{ry+=drawRow(d,t,ry)});
      y=ry;
...
export async function downloadRevenuePdfA4(report:MonthlyRevenueReport,month:number,year:number){
  const d=new jsPDF({orientation:'portrait',unit:'mm',format:'a4'});const generatedAt=new Date().toLocaleString('en-IN');
  ...
  for(let p=1;p<=d.getNumberOfPages();p++){d.setPage(p);d.setFont(FONT,'normal');d.setFontSize(7);d.setTextColor(80,80,80);d.text(`Page ${p} of ${d.getNumberOfPages()}`,PAGE_W-M,FOOT_Y,{align:'right'})}
