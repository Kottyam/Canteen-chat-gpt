...
function header(d:jsPDF,r:MonthlyRevenueReport,m:number,y:number,generatedAt:string){
  ...
  d.setTextColor(35,35,35);d.setFont(FONT,'normal');d.setFontSize(10);d.text('Canteen Management System',PAGE_W/2,22);
  d.setFont(FONT,'bold');d.setFontSize(14);d.text('REVENUE REPORT',PAGE_W/2,31,{align:'center'});
  d.setFont(FONT,'normal');d.setFontSize(9);const canteenLines=split(d,`Canteen Name: ${r.canteen_name}`,105);canteenLines.forEach((line,i)=>d.text(line,M,43+i*3.8));
  d.text(`Month: ${new Date(y,m-1,1).toLocaleString('en-IN',{month:'long',year:'numeric'})}`,M,49+(canteenLines.length-1)*3.8);d.text(`Period: ${formatReportDate(r.start_date)} to ${formatReportDate(r.end_date)}`,M,55+(canteenLines.length-1)*3.8);
  d.setFontSize(8);const generatedLines=split(d,`Generated: ${generatedAt}`,65);generatedLines.forEach((line,i)=>d.text(line,PAGE_W-M,43+i*3.8,{align:'right'}));return BODY_TOP;
}
function tableHead(d:jsPDF,y:number){d.setFillColor(245,247,246);d.setDrawColor(...BORDER);d.setLineWidth(.25);d.rect(M,y,PRINT_W,HEADER_H,'FD');d.setFont(FONT,'bold');d.setFontSize(HEADER_SIZE);d.setTextColor(35,35,35);['Date','Particulars','Quantity','Type','Total Amount'].forEach((h,i)=>{d.text(h,centerX(i),y+5.4,{align:'center'});if(i<4)d.line(COL_X[i]+COL_WIDTHS[i],y,COL_X[i]+COL_WIDTHS[i],y+HEADER_H)});return y+HEADER_H;}
...
function drawMoneyCentered(d:jsPDF,amount:number,column:number,y:number,h:number){const text=`Rs ${moneyValue(amount)}`;setBodyFont(d,false);d.text(text,centerX(column),y+(h-ROW_LINE)/2+2.9,{align:'center'});}
