...
/** Deterministic currency split. 0% means the member pays the full eligible gross amount. */
export function calculateEmployeeFoodContribution(grossAmount:number,employeeContributionPercentage:number):EmployeeFoodContribution{
 ...
 if(employeePercentage===0)return{grossAmount:gross,employeePercentage:0,employeeAmount:gross,companyPercentage:0,companyAmount:0,contributionMode:'percentage'};
