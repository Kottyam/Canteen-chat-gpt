...
export interface SubscriptionPlan{id:string;name:string;description:string|null;pricing_model:PricingModel;price:number;billing_period:BillingPeriod;currency:string;trial_days:number;active:boolean;is_default:boolean;min_members:number|null;max_members:number|null;created_at:string;updated_at:string}
...
export async function submitSubscriptionPayment(subscriptionId:string,amount:number,reference:string,paymentDate:string,note?:string){const {data,error}=await requireClient().rpc('canteen_submit_subscription_payment',{p_subscription_id:subscriptionId,p_amount:amount,p_reference:reference,p_payment_date:paymentDate,p_note:note||null});if(error)throw error;return data as SubscriptionPayment}
...
export async function loadOwnSubscription(canteenId:string){await syncSubscriptionStatuses(canteenId);const c=requireClient();const [{data:subscription,error},{data:payments,paymentError},{data:plans,planError},{data:settings,settingsError},{count:memberCount, error:memberError}]=await Promise.all([c.from('canteen_subscriptions').select('*').eq('canteen_id',canteenId).maybeSingle(),c.from('subscription_payments').select('*').eq('canteen_id',canteenId).order('created_at',{ascending:false}),c.from('subscription_plans').select('*').eq('active',true).order('price'),c.from('subscription_payment_settings').select('*').eq('id',true).maybeSingle(),c.from('profiles').select('id',{count:'exact',head:true}).eq('canteen_id',canteenId).eq('role','employee').eq('status','active')]);if(error)throw error;if(paymentError)throw paymentError;if(planError)throw planError;if(settingsError)throw settingsError;if(memberError)throw memberError;return{subscription:subscription as CanteenSubscription|null,payments:(payments||[]) as SubscriptionPayment[],plans:(plans||[]) as SubscriptionPlan[],settings:settings as SubscriptionPaymentSettings|null,memberCount:memberCount||0}}
export async function loadPlatformData(){await syncSubscriptionStatuses();const c=requireClient();const [stats,can,profiles,plans,subs,payments,settings]=await Promise.all([getPlatformStats(),c.from('canteens').select('id,name,owner_id,created_at,updated_at,archived').order('name'),c.from('profiles').select('id,employee_code,sr_number,full_name,role,admin_role,status,canteen_id').order('full_name'),c.from('subscription_plans').select('*').order('created_at',{ascending:false}),c.from('canteen_subscriptions').select('*'),c.from('subscription_payments').select('*').order('created_at',{ascending:false}),c.from('subscription_payment_settings').select('*').eq('id',true).maybeSingle()]);for(const x of [can,profiles,plans,subs,payments,settings])if(x.error)throw x.error;return{stats,canteens:can.data||[],profiles:profiles.data||[],plans:(plans.data||[]) as SubscriptionPlan[],subscriptions:(subs.data||[]) as CanteenSubscription[],payments:(payments.data||[]) as SubscriptionPayment[],settings:settings.data as SubscriptionPaymentSettings|null}}
export async function createPlan(input:Pick<SubscriptionPlan,'name'|'description'|'pricing_model'|'min_members'|'max_members'|'price'|'billing_period'|'currency'|'trial_days'|'active'|'is_default'>){const {data,error}=await requireClient().from('subscription_plans').insert(input).select('*').single();if(error)throw error;return data as SubscriptionPlan}
