...
const DEFAULT_MEMBER_CONTRIBUTION_PERCENTAGE=100;

export async function loadEmployeeFoodContributionPercentage():Promise<number>{
  if(!supabaseEnabled||!supabase)return DEFAULT_MEMBER_CONTRIBUTION_PERCENTAGE;
  const{data,error}=await supabase.rpc('get_employee_food_contribution_percentage');
  if(error)throw error;
  return normalizeEmployeeContributionPercentage(data==null?DEFAULT_MEMBER_CONTRIBUTION_PERCENTAGE:Number(data));
}
...
export async function saveEmployeeFoodContributionPercentage(value:number):Promise<number>{
  ...
  if(error)throw error;
  return normalizeEmployeeContributionPercentage(data==null?percentage:Number(data));
}
...
export async function loadEmployeeFoodContributionSettingsForAdmin():Promise<EmployeeFoodContributionSettings[]>{
  ...
  if(error)throw error;
  return(data||[]).map((row:any)=>({employeeId:String(row.employee_id),employeeName:String(row.employee_name||''),employeeCode:row.employee_code?String(row.employee_code):undefined,contributionMode:(row.contribution_mode==='fixed_amount'?'fixed_amount':'percentage') as EmployeeFoodContributionMode,employeeContributionPercentage:normalizeEmployeeContributionPercentage(row.employee_contribution_percentage==null?DEFAULT_MEMBER_CONTRIBUTION_PERCENTAGE:Number(row.employee_contribution_percentage)),fixedMonthlyAmount:Math.max(0,Number(row.fixed_monthly_amount||0))}));
}
...
export async function loadAdminEmployeeFoodContributionSettings():Promise<AdminEmployeeFoodContributionSetting[]>{
  ...
  return(data||[]).map((row:any)=>({
    ...
    employeeContributionPercentage:normalizeEmployeeContributionPercentage(row.employee_contribution_percentage==null?DEFAULT_MEMBER_CONTRIBUTION_PERCENTAGE:Number(row.employee_contribution_percentage)),
...
export async function saveAdminEmployeeFoodContributionSetting(employeeId:string|null,mode:EmployeeFoodContributionMode,percentage:number,fixedMonthlyAmount:number):Promise<AdminEmployeeFoodContributionSetting>{
  ...
  if(error)throw error;
  const row=(data||[])[0]||{};
  return{scope:employeeId?'employee':'all',employeeId:employeeId||'all',contributionMode:(row.contribution_mode==='fixed_amount'?'fixed_amount':'percentage') as EmployeeFoodContributionMode,employeeContributionPercentage:normalizeEmployeeContributionPercentage(row.employee_contribution_percentage==null?(mode==='percentage'?percentage:DEFAULT_MEMBER_CONTRIBUTION_PERCENTAGE):Number(row.employee_contribution_percentage)),fixedMonthlyAmount:Math.max(0,Number(row.fixed_monthly_amount||0)),hasIndividualOverride:Boolean(row.has_individual_override)};
}
...
export async function saveEmployeeFoodContributionSettings(employeeId:string,mode:EmployeeFoodContributionMode,percentage:number,fixedMonthlyAmount:number):Promise<EmployeeFoodContributionSettings>{
  ...
  if(error)throw error;
  const row=(data||[])[0]||{};
  return{employeeId,contributionMode:(row.contribution_mode==='fixed_amount'?'fixed_amount':'percentage') as EmployeeFoodContributionMode,employeeContributionPercentage:normalizeEmployeeContributionPercentage(row.employee_contribution_percentage==null?(mode==='percentage'?percentage:DEFAULT_MEMBER_CONTRIBUTION_PERCENTAGE):Number(row.employee_contribution_percentage)),fixedMonthlyAmount:Math.max(0,Number(row.fixed_monthly_amount||0))};
}
...
export async function previewEmployeeFoodContribution(employeeId:string,grossAmount:number,orderDate:string):Promise<EmployeeFoodContributionPreview>{
  ...
  if(error)throw error;
  return{contribution_mode:(data?.contribution_mode==='fixed_amount'?'fixed_amount':'percentage') as EmployeeFoodContributionMode,employee_contribution_percentage:data?.employee_contribution_percentage==null?DEFAULT_MEMBER_CONTRIBUTION_PERCENTAGE:Number(data.employee_contribution_percentage),fixed_monthly_amount:Number(data?.fixed_monthly_amount||0),previous_eligible_gross:Number(data?.previous_eligible_gross||0),gross_amount:Number(data?.gross_amount||0),company_amount:Number(data?.company_amount||0),employee_amount:Number(data?.employee_amount||0),remaining_allowance_before:Number(data?.remaining_allowance_before||0)};
}
