...
export async function loadSupabaseData() {
  ...
  const [{data:profiles,error:pErr},{data:orders,error:oErr},{data:menu,error:mErr},{data:holidays,error:hErr}] = await Promise.all([
    ...
    supabase.from('menu_prices').select('item_code,item_name,unit_price,active,updated_at').order('item_code'),
...
export async function upsertPrices(prices:Prices,menuItems:MenuItem[]=[]){if(!supabaseEnabled||!supabase)return;const map=new Map(menuItems.map(i=>[i.itemCode,i]));const rows=itemCodes.map(k=>{const i=map.get(k);return{item_code:k,item_name:i?.itemName||defaultNames[k]||k,unit_price:prices[k],active:i?.active??true};});const {error}=await supabase.from('menu_prices').upsert(rows,{onConflict:'item_code'});if(error)throw error;}
export async function saveMenuItem(item:MenuItem){if(!supabaseEnabled||!supabase)return;const {error}=await supabase.from('menu_prices').upsert({item_code:item.itemCode,item_name:item.itemName,unit_price:item.unitPrice,active:item.active},{onConflict:'item_code'});if(error)throw error;}
...
export async function activateMenuItem(itemCode:string){if(!supabaseEnabled||!supabase)return;const {error}=await supabase.from('menu_prices').update({active:true}).eq('item_code',itemCode);if(error)throw error;}
