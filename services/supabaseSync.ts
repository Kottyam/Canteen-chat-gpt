...
export async function loadSupabaseData(){
  ...
  const[{data:profiles,error:pErr},{data:orders,error:oErr},{data:menu,error:mErr},{data:holidays,error:hErr},{data:dailyMenu,error:dmErr}]=await Promise.all([
    ...
    supabase.from('orders').select('id,employee_id,ordered_for,status,created_at,updated_at,cancelled_at,order_items(id,item_code,item_name,quantity,unit_price)').order('ordered_for',{ascending:false}),
...
export async function upsertDailyMenu(menuDate:string,items:MenuItem[]){
  if(!supabaseEnabled||!supabase)return;
  const rows=items.map(i=>({menu_date:menuDate,item_code:i.itemCode,item_name:i.itemName.trim(),unit_price:Number(i.unitPrice),active:Boolean(i.active),updated_at:new Date().toISOString()}));
  if(!rows.length)return;
  const{error}=await supabase.from('daily_menu').upsert(rows,{onConflict:'menu_date,item_code'});if(error)throw error;
}
...
export async function getDailyMenu(menuDate:string):Promise<DailyMenuItem[]>{
  if(!supabaseEnabled||!supabase)return [];
  const{data,error}=await supabase.from('daily_menu').select('menu_date,item_code,item_name,unit_price,active').eq('menu_date',menuDate).order('item_code');
  if(error)throw error;
  return(data||[]).map((r:any)=>({menuDate:String(r.menu_date),itemCode:r.item_code,itemName:r.item_name,unitPrice:Number(r.unit_price),active:Boolean(r.active)}));
}
