-- Revenue reporting only: keep Guest as a separate financial category,
-- but never split normal food rows by employee/admin order source.
-- Source transactions are not modified.
...
begin
  ...
  select
    coalesce(sum(case when oi.item_source='guest' then 0 else coalesce(oi.line_total,oi.quantity*oi.unit_price) end),0),
  ...
  with food_rows as (
    select
      o.ordered_for as date,
      coalesce(nullif(trim(oi.item_name),''),'Food Order') particulars,
      ...
      case when oi.item_source='guest' then 2 else 1 end sort_order
    from public.orders o join public.order_items oi on oi.order_id=o.id
    where o.canteen_id=v_canteen and o.ordered_for>=v_start and o.ordered_for<v_end
      and coalesce(o.status,'')<>'cancelled' and oi.canteen_id=v_canteen
    group by
      o.ordered_for,
      oi.item_code,
      coalesce(nullif(trim(oi.item_name),''),'Food Order'),
      case when oi.item_source='guest' then 'Guest Revenue' else 'Food Revenue' end
  ),
  admin_rows as (
    select a.adjustment_date date,'Admin Added' particulars,'Admin Added' type,
      count(*)::numeric quantity,sum(a.amount)::numeric amount,3 sort_order
    from public.employee_adjustments a
    where a.canteen_id=v_canteen and a.adjustment_date>=v_start and a.adjustment_date<v_end
    group by a.adjustment_date
  ),
  additional_rows as (
    select r.revenue_date date,coalesce(nullif(trim(r.description),''),'Additional Revenue') particulars,
      'Additional Revenue' type,null::numeric quantity,sum(r.amount)::numeric amount,4 sort_order
    from public.additional_revenues r
    where r.canteen_id=v_canteen and r.revenue_date>=v_start and r.revenue_date<v_end
    group by r.revenue_date,coalesce(nullif(trim(r.description),''),'Additional Revenue')
  ),
  expense_rows as (
    select e.expense_date date,coalesce(nullif(trim(e.description),''),'Expense') particulars,
      'Expense' type,null::numeric quantity,sum(e.amount)::numeric amount,5 sort_order
    from public.expenses e
    where e.canteen_id=v_canteen and e.expense_date>=v_start and e.expense_date<v_end
    group by e.expense_date,coalesce(nullif(trim(e.description),''),'Expense')
  )
  select coalesce(jsonb_agg(jsonb_build_object('date',z.date,'particulars',z.particulars,'quantity',z.quantity,'type',z.type,'amount',z.amount)
    order by z.date,z.sort_order,z.particulars),'[]'::jsonb)
  into v_transactions
  ...
  select coalesce(jsonb_agg(jsonb_build_object('date',e.expense_date,'particulars',coalesce(nullif(trim(e.description),''),'Expense'),'type','Expense','amount',e.amount)
    order by e.expense_date,e.description),'[]'::jsonb)
  into v_expense_rows
  from public.expenses e
  where e.canteen_id=v_canteen and e.expense_date>=v_start and e.expense_date<v_end;

  return jsonb_build_object(
    'canteen_name',coalesce(v_canteen_name,'Go Canteen'),'start_date',v_start,'end_date',(v_end-1),
    ...
    'net_revenue',v_food+v_guest+v_admin+v_additional-v_expenses,
    'transactions',v_transactions,'expenses',v_expense_rows);
end;$$;
