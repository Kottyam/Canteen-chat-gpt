-- Fix incremental monthly bill publishing to use the business/order date boundary.
-- The authoritative incremental boundary is:
--   ordered_for / adjustment_date > last_covered_through
--   AND <= the selected billing period end date.
-- created_at is used only as a legacy fallback when a payment has no covered_through.
