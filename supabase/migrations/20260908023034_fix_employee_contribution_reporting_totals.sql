...
with bounds as (
  ...
  select p.id from public.profiles p where p.role='employee' and p.status<>'deleted' and p.canteen_id=public.current_canteen_id()
),bills as (
  select b.id,b.employee_id from public.monthly_bills b where b.bill_month=p_month and b.bill_year=p_year and b.canteen_id=public.current_canteen_id()
),activity as (
  select e.id employee_id,
    coalesce((select sum(q.employee_amount) from (
      ...
      where o.employee_id=e.id and o.canteen_id=public.current_canteen_id() and o.status='active' and o.ordered_for between bounds.start_date and bounds.end_date and oi.item_source<>'guest'
      group by o.id
    ) q),0)+coalesce((select sum(a.amount) from public.employee_adjustments a where a.employee_id=e.id and a.canteen_id=public.current_canteen_id() and a.adjustment_date between bounds.start_date and bounds.end_date),0) current_total,
    coalesce((select sum(q.employee_amount) from (
      select o.id,coalesce(max(o.employee_food_amount),sum(coalesce(oi.line_total,oi.quantity*oi.unit_price))) employee_amount
      from public.orders o join public.order_items oi on oi.order_id=o.id
      where o.employee_id=e.id and o.canteen_id=public.current_canteen_id() and o.status='active' and o.ordered_for between bounds.start_date and bounds.end_date and oi.item_source<>'guest'
        and o.created_at>coalesce((select max(coalesce(p2.covered_through_at,p2.created_at)) from public.bill_payments p2 where p2.bill_id=(select b2.id from public.monthly_bills b2 where b2.employee_id=e.id and b2.bill_month=p_month and b2.bill_year=p_year and b2.canteen_id=public.current_canteen_id() limit 1)),'-infinity'::timestamptz)
      group by o.id
    ) q),0)+coalesce((select sum(a.amount) from public.employee_adjustments a where a.employee_id=e.id and a.canteen_id=public.current_canteen_id() and a.adjustment_date between bounds.start_date and bounds.end_date
...
select e.id,b.id,a.current_total,coalesce(r.requested_total,0),case when r.last_covered_at is null then a.current_total else greatest(a.new_activity_total,0) end,r.last_covered_at,r.last_covered_through,
  case when r.last_covered_at is null then a.current_total>0 else greatest(a.new_activity_total,0)>0 end,
  case when r.last_covered_at is null and a.current_total<=0 then 'No payable amount available.' when r.last_covered_at is not null and greatest(a.new_activity_total,0)<=0 then 'No new amount since the last publication.' else null end
from employees e cross join bounds left join bills b on b.employee_id=e.id left join activity a on a.employee_id=e.id
left join lateral(select coalesce((select sum(p.amount) from public.bill_payments p where p.bill_id=b.id and p.employee_id=e.id and p.canteen_id=public.current_canteen_id()),0) requested_total,
...
(select p3.covered_through from public.bill_payments p3 where p3.bill_id=b.id and p3.employee_id=e.id and p3.canteen_id=public.current_canteen_id() order by coalesce(p3.covered_through_at,p3.created_at) desc,p3.created_at desc,p3.id desc limit 1) last_covered_through) r on true;
$$;
