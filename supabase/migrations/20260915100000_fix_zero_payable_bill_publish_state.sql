-- Bill publishing eligibility must use the existing persisted member-payable values.
-- A positive eligible food amount can legitimately produce a zero member payable
-- when company contribution covers the full eligible amount. That is a valid
-- no-bill state, not a publication failure.
...
select e.id,b.id,totals.current_total,coalesce(req.requested_total,0),
       ...
       case when req.last_covered_at is null and totals.current_total<=0 then 'No bill required — Member payable is ₹0.'
...
left join lateral (
  select
    coalesce((select sum(coalesce(o.employee_food_amount,0)) from public.orders o where o.employee_id=e.id and o.canteen_id=public.current_canteen_id() and o.status='active' and o.ordered_for between bounds.start_date and bounds.end_date),0)
    + coalesce((select sum(coalesce(oi.line_total,oi.quantity*oi.unit_price)) from public.orders o join public.order_items oi on oi.order_id=o.id where o.employee_id=e.id and o.canteen_id=public.current_canteen_id() and o.status='active' and o.ordered_for between bounds.start_date and bounds.end_date and oi.item_source='guest'),0)
