-- Admin Member/Staff additional amounts are part of the selected date's
-- Order History. They are not exposed as a linked/standalone choice.
-- Delete the complete date history, including all employee adjustments.
...
begin
  ...
  from public.orders o where o.canteen_id=v_canteen and o.ordered_for=p_date;

  select coalesce(array_agg(a.id),'{}'::uuid[]) into v_adjustment_ids
  from public.employee_adjustments a where a.canteen_id=v_canteen and a.adjustment_date=p_date;
  ...
  from public.order_items oi where coalesce(array_length(v_order_ids,1),0)>0 and oi.order_id=any(v_order_ids) and oi.canteen_id=v_canteen;

  if coalesce(array_length(v_adjustment_ids,1),0)>0 then
    select coalesce(sum(greatest(coalesce(a.amount,0),0)),0) into v_deleted_admin_amount
    from public.employee_adjustments a where a.id=any(v_adjustment_ids) and a.canteen_id=v_canteen;
  end if;

  if coalesce(array_length(v_order_ids,1),0)>0 or coalesce(array_length(v_adjustment_ids,1),0)>0 then
    delete from public.notifications n where n.canteen_id=v_canteen and
      ((n.payload ? 'order_id' and (n.payload->>'order_id')::uuid=any(v_order_ids))
       or (n.payload ? 'adjustment_id' and (n.payload->>'adjustment_id')::uuid=any(v_adjustment_ids)));
  ...
  for r in select * from _gocanteen_history_impact loop
    select count(distinct o.ordered_for)::integer into v_days from public.orders o
    where o.employee_id=r.employee_id and o.canteen_id=v_canteen
      and extract(month from o.ordered_for)=r.bill_month and extract(year from o.ordered_for)=r.bill_year
      and coalesce(o.status,'')<>'cancelled';
    update public.monthly_bills mb set
      food_total=greatest(coalesce(mb.food_total,0)-coalesce(r.food_total,0),0),
      ...
      days_ordered=coalesce(v_days,0),updated_at=now()
    where mb.employee_id=r.employee_id and mb.canteen_id=v_canteen and mb.bill_month=r.bill_month and mb.bill_year=r.bill_year and mb.published=false;
  end loop;
