-- Restore authenticated execution for the existing member-notification RPC.
-- Keep the existing notification implementation, but require the same admin
-- permission families already used by the notifications RLS policy and keep
-- the recipient inside the caller's current canteen.
create or replace function public.create_member_notification(p_recipient_id uuid,p_notification_type text,p_title text,p_message text,p_payload jsonb default '{}'::jsonb,p_event_key text default null)
returns uuid
language plpgsql
security definer
