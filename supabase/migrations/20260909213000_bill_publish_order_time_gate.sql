-- Bill publishing eligibility only; existing billing/payment calculations remain unchanged.
-- Previous-day bills are allowed. Today's bill requires today's existing Order Time to have closed.
...
begin
  ...
  if public.is_holiday_for_date(v_now::date) then return true; end if;
  select enabled,end_time into v_enabled,v_end from public.order_window_settings where canteen_id=public.current_canteen_id();
  if not found or not coalesce(v_enabled,false) or v_end is null then return false; end if;
  return v_now::time >= v_end;
end;
$$;

create or replace function public.bill_publish_order_time_state(p_bill_date date)
...
-- Add the eligibility gate before the existing publish calculation/insert path.
