...
begin
  ...
  v_is_admin:=v_profile.role='admin'
    and v_profile.canteen_id is not null
    and v_profile.canteen_id=v_order.canteen_id;
  ...
  if not v_is_admin
     and p_source='employee'
     and coalesce(v_order.order_source,'employee')='admin' then
    if not public.employee_order_window_open() then
      raise exception 'Order window is currently closed';
    end if;
  end if;

  v_item_source:=case
    when p_source='employee'
      and coalesce(v_order.order_source,'employee')='admin'
      then 'admin'
    when v_is_admin
      and p_source='employee'
      and coalesce(v_order.order_source,'employee')='admin'
  ...
  if v_remaining_items=0 then
    if v_is_admin then
      ...
      v_message:=format(
        'Your order for %s was cancelled by Admin.',
        to_char(v_order.ordered_for,'DD Mon YYYY')
      );
      ...
      perform public.create_member_notification(
        ...
        jsonb_build_object(
          ...
          'order_date',v_order.ordered_for::text,
  ...
  if v_is_admin then
    ...
    v_message:=format(
      'Your %s order for %s was cancelled by Admin.',
      case when p_source='guest' then 'guest' else 'member' end,
      to_char(v_order.ordered_for,'DD Mon YYYY')
    );
    v_event_key:=format(
      'order_part_cancelled:%s:%s',
      v_order.id::text,
      p_source
    );
    perform public.create_member_notification(
