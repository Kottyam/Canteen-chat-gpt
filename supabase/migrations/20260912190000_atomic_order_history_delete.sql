...
begin
  ...
  if v_canteen is null then
    raise exception 'Canteen context required';
  end if;
  if p_date is null then
    raise exception 'Order history date is required';
  end if;
  ...
  if coalesce(array_length(v_order_ids,1),0)>0 then
    ...
    where a.canteen_id=v_canteen
      and a.order_id=any(v_order_ids);
  end if;
  ...
  create temporary table if not exists _gocanteen_history_impact (
    ...
    guest_total numeric not null default 0,
  ...
  if coalesce(array_length(v_adjustment_ids,1),0)>0 then
    ...
    select a.employee_id,
           extract(month from a.adjustment_date)::integer,
           extract(year from a.adjustment_date)::integer,
           0,
           0,
           coalesce(sum(greatest(coalesce(a.amount,0),0)),0)
    from public.employee_adjustments a
    where a.canteen_id=v_canteen
      and a.id=any(v_adjustment_ids)
    group by a.employee_id,a.adjustment_date
    on conflict(employee_id) do update
      set admin_total=_gocanteen_history_impact.admin_total+excluded.admin_total;
  end if;

  select coalesce(sum(case when oi.item_source<>'guest' then greatest(coalesce(oi.line_total,oi.quantity*oi.unit_price),0) else 0 end),0),
         coalesce(sum(case when oi.item_source='guest' then greatest(coalesce(oi.line_total,oi.quantity*oi.unit_price),0) else 0 end),0)
    into v_deleted_order_amount,v_deleted_guest_amount
  ...
  if coalesce(array_length(v_adjustment_ids,1),0)>0 then
    select coalesce(sum(greatest(coalesce(a.amount,0),0)),0)
      into v_deleted_admin_amount
    from public.employee_adjustments a
    where a.id=any(v_adjustment_ids)
      and a.canteen_id=v_canteen;
  end if;

  -- Remove only notifications tied to the exact deleted order/adjustment IDs.
  -- Unrelated notifications, including standalone Admin Added entries, remain.
  if coalesce(array_length(v_order_ids,1),0)>0 or coalesce(array_length(v_adjustment_ids,1),0)>0 then
    ...
    where n.canteen_id=v_canteen
      and (
        (n.payload ? 'order_id' and (n.payload->>'order_id')::uuid=any(v_order_ids))
        or
        (n.payload ? 'adjustment_id' and (n.payload->>'adjustment_id')::uuid=any(v_adjustment_ids))
      );
    get diagnostics v_deleted_notifications=row_count;
  end if;

  -- Explicitly delete linked adjustments first. Their FK also protects them if
  -- an order is deleted through another path, while standalone rows are safe.
  if coalesce(array_length(v_adjustment_ids,1),0)>0 then
  ...
  if coalesce(array_length(v_order_ids,1),0)>0 then
    ...
    where id=any(v_order_ids)
      and canteen_id=v_canteen
      and ordered_for=p_date;
    get diagnostics v_deleted_orders=row_count;
  end if;

  -- Keep published bills immutable. Draft monthly bills are maintained from the
  -- deleted transaction set only; standalone adjustments are never subtracted.
  for r in select * from _gocanteen_history_impact loop
    select count(distinct o.ordered_for)::integer
      into v_days
    from public.orders o
    where o.employee_id=r.employee_id
      and o.canteen_id=v_canteen
    ...
    set food_total=greatest(coalesce(mb.food_total,0)-coalesce(r.food_total,0),0),
        ...
        total=greatest(coalesce(mb.food_total,0)-coalesce(r.food_total,0),0)
             +greatest(coalesce(mb.admin_added_total,0)-coalesce(r.admin_total,0),0),
        days_ordered=coalesce(v_days,0),
        updated_at=now()
    where mb.employee_id=r.employee_id
      and mb.canteen_id=v_canteen
      and mb.bill_month=r.bill_month
      and mb.bill_year=r.bill_year
      and mb.published=false;
  end loop;
  ...
  return jsonb_build_object(
    ...
    'deleted_order_linked_adjustments',v_deleted_adjustments,
    'deleted_notifications',v_deleted_notifications,
    'deleted_member_food_amount',v_deleted_order_amount,
    'deleted_guest_food_amount',v_deleted_guest_amount,
    'deleted_admin_added_amount',v_deleted_admin_amount
  );
end;
$$;
