-- Revenue reporting only: use the actual order-item menu identity.
-- Guest remains a separate financial category; employee/admin order source is not a separate menu row.
...
begin
  ...
  with food_rows as (
    select o.ordered_for as date,min(coalesce(nullif(trim(oi.item_name),''),'Food Order')) particulars,
    ...
    group by o.ordered_for,coalesce(nullif(trim(oi.item_code),''),lower(nullif(trim(oi.item_name),''))),case when oi.item_source='guest' then 'Guest Revenue' else 'Food Revenue' end
  ),
  admin_rows as (
