...
as $$
declare m public.subscription_payment_methods; v_default_id uuid; active_count integer;
begin
...
if p_action='delete' then
 ...
 delete from public.subscription_payment_methods where id=p_method_id returning * into m;
 if m.is_default then
  select id into v_default_id from public.subscription_payment_methods where active order by created_at asc limit 1;
  if v_default_id is not null then update public.subscription_payment_methods set is_default=true,updated_at=now() where id=v_default_id; end if;
 end if;
 return m;
 ...
 update public.subscription_payment_methods set active=false,is_default=false,updated_at=now() where id=p_method_id returning * into m;
 if not found then raise exception 'Payment method not found'; end if;
 return m;
