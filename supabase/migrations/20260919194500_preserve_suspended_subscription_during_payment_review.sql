create or replace function public.canteen_submit_subscription_payment(
  p_subscription_id uuid,p_amount numeric,p_reference text,p_payment_date date
)
...
begin
  ...
  v_canteen_id:=public.current_canteen_id(); if v_canteen_id is null then raise exception 'Canteen is not configured'; end if;
  ...
  insert into public.subscription_payments(canteen_id,subscription_id,plan_id,amount,currency,payment_date,payment_status,transaction_reference)
  values(v_canteen_id,v_sub.id,v_sub.plan_id,p_amount,v_sub.currency,p_payment_date::timestamptz,'pending',trim(p_reference))
...
returns public.subscription_payments language plpgsql security definer set search_path = ''
as $$
declare v_payment public.subscription_payments; v_sub public.canteen_subscriptions; v_plan public.subscription_plans; v_start timestamptz; v_end timestamptz; v_original_status text;
begin
  ...
  if p_status='paid' then
    ...
    v_start:=case when v_sub.subscription_end is not null and v_sub.subscription_end>now() then v_sub.subscription_end else now() end;
    ...
    update public.subscription_payments set payment_status='paid',updated_at=now(),billing_period_start=v_start,billing_period_end=v_end where id=v_payment.id returning * into v_payment;
    update public.canteen_subscriptions set status='active',payment_status='paid',subscription_start=v_start,subscription_end=v_end,amount=v_payment.amount,currency=v_payment.currency,updated_at=now() where id=v_sub.id;
  else
    update public.subscription_payments set payment_status='failed',updated_at=now() where id=v_payment.id returning * into v_payment;
    update public.canteen_subscriptions set status=case when v_original_status='suspended' then 'suspended' else 'payment_pending' end,payment_status='failed',updated_at=now() where id=v_sub.id;
  end if;
  return v_payment;
end;$$;
