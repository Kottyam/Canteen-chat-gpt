...
as $$
declare v_canteen uuid;v_id uuid;v_payload jsonb:=coalesce(p_payload,'{}'::jsonb);
begin
  select canteen_id into v_canteen from public.profiles where id=p_recipient_id and role='employee' and status='active';
  ...
  insert into public.notifications(recipient_id,notification_type,title,message,payload,created_at,canteen_id) values(p_recipient_id,p_notification_type,p_title,p_message,v_payload,now(),v_canteen) on conflict do nothing returning id into v_id;
  return v_id;
end;
$$;

create or replace function public.notify_canteen_employees_for_canteen(p_canteen_id uuid,p_event_key text,p_notification_type text,p_title text,p_message text,p_payload jsonb default '{}'::jsonb)
...
create or replace function public.notify_canteen_employees(p_event_key text,p_notification_type text,p_title text,p_message text,p_payload jsonb default '{}'::jsonb)
returns integer language plpgsql security definer set search_path=public as $$
declare v_canteen uuid:=public.current_canteen_id();v_count integer:=0;r record;
begin
  if not public.is_admin_user() then raise exception 'Admin authorization required'; end if;
...
begin
  ...
  insert into public.holiday_dates(canteen_id,holiday_date,created_by) values(v_canteen,p_date,auth.uid()) on conflict(canteen_id,holiday_date) do nothing returning holiday_date into v_inserted;
  if v_inserted is not null then v_count:=public.notify_canteen_employees('holiday_declared:'||p_date::text||':'||gen_random_uuid()::text,'holiday_declared','Holiday Declared',format('%s is declared a holiday. Orders are not available on this date.',to_char(p_date,'DD Mon YYYY')),jsonb_build_object('holiday_date',p_date::text,'holiday_source','declared')); end if;
...
begin
  ...
  for r in select canteen_id,enabled,start_time,end_time from public.order_window_settings where coalesce(enabled,false) and start_time is not null and end_time is not null loop
    if to_char(v_now,'HH24:MI')=to_char(r.start_time,'HH24:MI') then perform public.notify_canteen_employees_for_canteen(r.canteen_id,'order_time_open:'||r.canteen_id::text||':'||v_today::text,'order_time_open','Order Time Open','Today''s orders are now open.',jsonb_build_object('business_date',v_today::text,'order_window_state','open')); end if;
    if to_char(v_now,'HH24:MI')=to_char(r.end_time,'HH24:MI') then perform public.notify_canteen_employees_for_canteen(r.canteen_id,'order_time_closed:'||r.canteen_id::text||':'||v_today::text,'order_time_closed','Order Time Closed','Today''s orders are now closed.',jsonb_build_object('business_date',v_today::text,'order_window_state','closed')); end if;
  end loop;
end;$$;
...
returns trigger language plpgsql security definer set search_path=public as $$
begin
  if new.status='pending_verification' and new.status is distinct from old.status then perform public.create_member_notification(new.employee_id,'payment_submitted','Payment Submitted','Your payment has been submitted for verification.',jsonb_build_object('bill_id',new.bill_id,'payment_id',new.id,'request_sequence',new.request_sequence,'amount',new.amount),format('payment_submitted:%s:%s',new.id,coalesce(new.confirmed_at::text,new.updated_at::text)));
  elsif new.status='rejected' and new.status is distinct from old.status then perform public.create_member_notification(new.employee_id,'payment_status_updated','Payment Rejected','Your payment was rejected. Please review the payment request and try again.',jsonb_build_object('bill_id',new.bill_id,'payment_id',new.id,'request_sequence',new.request_sequence,'status','rejected','amount',new.amount),format('payment_status:%s:rejected:%s',new.id,new.updated_at::text)); end if;
  return new;
end;$$;
...
begin
  ...
  if v_order.id is not null and v_order.status='active' and tg_op<>'INSERT' then perform public.create_member_notification(v_order.employee_id,'order_updated','Order Updated',format('Your order for %s was updated by Admin.',to_char(v_order.ordered_for,'DD Mon YYYY')),jsonb_build_object('order_id',v_order.id,'order_date',v_order.ordered_for::text),format('order_updated:%s:%s',v_order.id,txid_current()::text)); end if;
