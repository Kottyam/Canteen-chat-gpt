-- Strictly incremental monthly bill publishing.
-- Reuses monthly_bills + bill_payments. Each payment batch records the exact
-- publication timestamp used as the next billing cutoff.

alter table public.bill_payments
  add column if not exists covered_through_at timestamptz;

update public.bill_payments
set covered_through_at = coalesce(covered_through_at, created_at)
where covered_through_at is null;

create index if not exists bill_payments_bill_coverage_idx
  on public.bill_payments (bill_id, covered_through_at desc);
...
employees as (
  ...
  where p.role='employee' and p.status<>'deleted'
...
select e.id,b.id,totals.current_total,coalesce(req.requested_total,0),
       case when req.last_covered_at is null then totals.current_total
            else greatest(totals.new_activity_total,0) end,
       req.last_covered_at,req.last_covered_through,
       case when req.last_covered_at is null then totals.current_total>0
            else greatest(totals.new_activity_total,0)>0 end,
...
left join lateral (
  select
    ...
    coalesce((select sum(coalesce(oi.line_total,oi.quantity*oi.unit_price))
              from public.orders o join public.order_items oi on oi.order_id=o.id
              where o.employee_id=e.id and o.canteen_id=public.current_canteen_id()
                and o.status='active' and o.ordered_for between bounds.start_date and bounds.end_date
                and o.created_at > coalesce((select max(coalesce(p2.covered_through_at,p2.created_at))
                                             from public.bill_payments p2 where p2.bill_id=b.id),'-infinity'::timestamptz)),0)
    + coalesce((select sum(a.amount) from public.employee_adjustments a
                where a.employee_id=e.id and a.canteen_id=public.current_canteen_id()
                  and a.adjustment_date between bounds.start_date and bounds.end_date
                  and a.created_at > coalesce((select max(coalesce(p3.covered_through_at,p3.created_at))
                                               from public.bill_payments p3 where p3.bill_id=b.id),'-infinity'::timestamptz)),0) new_activity_total
) totals on true
left join lateral (
  select
...
declare
  ...
  v_guest_food numeric:=0;
  v_admin_added numeric:=0;
  v_total numeric:=0;
  v_new_activity numeric:=0;
  v_last_cutoff timestamptz;
  v_seq integer:=1;
  v_id uuid;
  v_ref text;
begin
  ...
  -- Serialize publication for exactly one employee/month/canteen resource.
  ...
  -- Recalculate from the persisted billable activity. Client totals are retained
  -- in the API for compatibility but are not trusted for publication decisions.
  select
  ...
  select max(coalesce(p.covered_through_at,p.created_at)) into v_last_cutoff
  ...
  where b.employee_id=p_employee_id and b.bill_month=p_month and b.bill_year=p_year
    and b.canteen_id=public.current_canteen_id() and p.employee_id=p_employee_id;

  if v_last_cutoff is null then
    v_new_activity:=v_total;
  else
    select coalesce(sum(coalesce(oi.line_total,oi.quantity*oi.unit_price)),0) into v_new_activity
    from public.orders o join public.order_items oi on oi.order_id=o.id
    where o.employee_id=p_employee_id and o.canteen_id=public.current_canteen_id()
      and o.status='active' and o.ordered_for between v_start and v_end
      and o.created_at>v_last_cutoff;

    select v_new_activity+coalesce(sum(a.amount),0) into v_new_activity
    from public.employee_adjustments a
    where a.employee_id=p_employee_id and a.canteen_id=public.current_canteen_id()
      and a.adjustment_date between v_start and v_end and a.created_at>v_last_cutoff;
  end if;

  if v_last_cutoff is not null and greatest(v_new_activity,0)<=0 then
  ...
  insert into public.monthly_bills(
    employee_id,bill_month,bill_year,billing_start_date,billing_end_date,
    days_ordered,food_total,guest_food_total,admin_added_total,total,published,
    published_at,updated_at,canteen_id,upi_name,upi_id,upi_number)
  values(
  ...
  values(
    ...
    greatest(case when v_last_cutoff is null then v_total else v_new_activity end,0),
