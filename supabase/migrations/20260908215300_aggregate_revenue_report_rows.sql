...
-- Food/guest rows are grouped by business date, particular and source, with quantity
-- representing distinct orders. Admin adjustments are grouped per date; additional
-- revenue and expenses preserve their meaningful particulars.
...
begin
  ...
  with food_rows as (
    select o.ordered_for as date,
      coalesce(oi.item_name,'Food Order') as particulars,
      case when oi.item_source='guest' then 'Guest Revenue' else 'Food Revenue' end as type,
      count(distinct o.id)::numeric as quantity,
      sum(oi.quantity*oi.unit_price)::numeric as amount,
      case when oi.item_source='guest' then 2 else 1 end as sort_order
    from public.orders o
    join public.order_items oi on oi.order_id=o.id
    where o.canteen_id=v_canteen and o.ordered_for>=v_start and o.ordered_for<v_end
      and coalesce(o.status,'')<>'cancelled' and oi.canteen_id=v_canteen
    group by o.ordered_for,coalesce(oi.item_name,'Food Order'),oi.item_source
  ), admin_rows as (
    select a.adjustment_date as date,'Admin Added'::text particulars,'Admin Added'::text type,
    ...
    select r.revenue_date as date,coalesce(nullif(r.description,''),'Additional Revenue') particulars,
      'Additional Revenue'::text type,null::numeric quantity,sum(r.amount)::numeric amount,4 sort_order
    from public.additional_revenues r
    where r.canteen_id=v_canteen and r.revenue_date>=v_start and r.revenue_date<v_end
    group by r.revenue_date,coalesce(nullif(r.description,''),'Additional Revenue')
  ), expense_rows as (
    select e.expense_date as date,coalesce(nullif(e.description,''),'Expense') particulars,
      'Expense'::text type,null::numeric quantity,sum(e.amount)::numeric amount,5 sort_order
    from public.expenses e
    where e.canteen_id=v_canteen and e.expense_date>=v_start and e.expense_date<v_end
    group by e.expense_date,coalesce(nullif(e.description,''),'Expense')
  )
  select coalesce(jsonb_agg(jsonb_build_object('date',z.date,'particulars',z.particulars,'quantity',z.quantity,'type',z.type,'amount',z.amount) order by z.date,z.sort_order,z.particulars),'[]'::jsonb)
    into v_transactions
  from (
    select * from food_rows union all select * from admin_rows union all select * from additional_rows union all select * from expense_rows
  ) z;

  select coalesce(jsonb_agg(jsonb_build_object('date',e.expense_date,'particulars',coalesce(nullif(e.description,''),'Expense'),'type','Expense','amount',e.amount) order by e.expense_date,e.description), '[]'::jsonb)
