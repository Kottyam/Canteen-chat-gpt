...
as $$
declare v_canteen uuid; v_canteen_name text; v_start date; v_end date; v_food numeric:=0; v_guest numeric:=0; v_admin numeric:=0; v_additional numeric:=0; v_expenses numeric:=0; v_transactions jsonb; v_expense_rows jsonb;
begin
 ...
 select coalesce(sum(case when oi.item_source='guest' then 0 else coalesce(oi.line_total,oi.quantity*oi.unit_price) end),0),coalesce(sum(case when oi.item_source='guest' then coalesce(oi.line_total,oi.quantity*oi.unit_price) else 0 end),0) into v_food,v_guest from public.orders o join public.order_items oi on oi.order_id=o.id where o.canteen_id=v_canteen and o.ordered_for>=v_start and o.ordered_for<v_end and coalesce(o.status,'')<>'cancelled' and oi.canteen_id=v_canteen;
 ...
 with food_rows as (
  select o.ordered_for date,coalesce(nullif(oi.item_name,''),'Food Order') particulars,case when oi.item_source='guest' then 'Guest Revenue' else 'Food Revenue' end type,sum(coalesce(oi.quantity,0))::numeric quantity,sum(coalesce(oi.line_total,oi.quantity*oi.unit_price))::numeric amount,case when oi.item_source='guest' then 2 else 1 end sort_order
  ...
  select r.revenue_date date,coalesce(nullif(r.description,''),'Additional Revenue') particulars,'Additional Revenue' type,null::numeric quantity,sum(r.amount)::numeric amount,4 sort_order from public.additional_revenues r where r.canteen_id=v_canteen and r.revenue_date>=v_start and r.revenue_date<v_end group by r.revenue_date,coalesce(nullif(r.description,''),'Additional Revenue')
 ), expense_rows as (
  select e.expense_date date,coalesce(nullif(e.description,''),'Expense') particulars,'Expense' type,null::numeric quantity,sum(e.amount)::numeric amount,5 sort_order from public.expenses e where e.canteen_id=v_canteen and e.expense_date>=v_start and e.expense_date<v_end group by e.expense_date,coalesce(nullif(e.description,''),'Expense')
 )
 select coalesce(jsonb_agg(jsonb_build_object('date',z.date,'particulars',z.particulars,'quantity',z.quantity,'type',z.type,'amount',z.amount) order by z.date,z.sort_order,z.particulars),'[]'::jsonb) into v_transactions from (select * from food_rows union all select * from admin_rows union all select * from additional_rows union all select * from expense_rows) z;
 select coalesce(jsonb_agg(jsonb_build_object('date',e.expense_date,'particulars',coalesce(nullif(e.description,''),'Expense'),'type','Expense','amount',e.amount) order by e.expense_date,e.description),'[]'::jsonb) into v_expense_rows from public.expenses e where e.canteen_id=v_canteen and e.expense_date>=v_start and e.expense_date<v_end;
